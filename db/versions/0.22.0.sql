...
DO $$
    ...
    BEGIN
        ...
        LOOP
            ...
            FROM beckett.subscription_groups sg
            INNER JOIN beckett.subscriptions s ON sg.id = s.subscription_group_id
            INNER JOIN beckett.checkpoints c ON sg.name = c.group_name AND s.name = c.name
            WHERE c.id > last_id
            AND c.id IN (
                SELECT c2.id
...
-- Step 10: Drop old checkpoints table and rename new one
DROP TABLE beckett.checkpoints CASCADE;
ALTER TABLE beckett.checkpoints_new RENAME TO checkpoints;

-- Step 11: Add constraints to new checkpoints table
...
ALTER TABLE beckett.checkpoints ADD CONSTRAINT checkpoints_subscription_id_fkey

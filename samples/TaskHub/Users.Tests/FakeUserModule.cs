...
public class FakeUserModule : IUserModule
{
    ...
    public Task Execute<TState>(ICommand<TState> command, CancellationToken cancellationToken) where TState : class, IApply, new()
    {
        if (_exception is not null)
        {
            throw _exception;

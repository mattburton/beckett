...
public sealed class StateGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        var source = context.SyntaxProvider.ForAttributeWithMetadataName(
                "Core.State.StateAttribute",
                predicate: static (node, _) => IsSyntaxTargetForGeneration(node),
                transform: static (context, _) => GetTargetForGeneration(context)
            )
            .Where(static m => m is not null);

        context.RegisterSourceOutput(source, (sourceContext, data) => Execute(data, sourceContext));
    }

    private static void Execute(State? state, SourceProductionContext context)
    {

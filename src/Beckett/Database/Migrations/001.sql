...
AS
$$
INSERT INTO __schema__.recurring_messages (application, name, cron_expression, stream_name, type, data, metadata, next_occurrence)
...
AS
$$
SELECT application, name, cron_expression, stream_name, type, data, metadata, next_occurrence, timestamp
FROM __schema__.recurring_messages
WHERE application = _application
AND complete = false
AND (next_occurrence IS NULL OR next_occurrence <= CURRENT_TIMESTAMP)
FOR UPDATE
SKIP LOCKED
LIMIT _batch_size;
$$;

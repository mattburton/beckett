...
public interface ISubscriptionConfigurationBuilder
{
    ...
    /// instance method or a static method. The parameters will determine what type of handler it is, i.e. if the
    /// method accepts an <see cref="IMessageBatch"/> then it is a batch handler, otherwise it handles individual
    /// messages via <see cref="IMessageContext"/> and / or an actual message type. Handler methods must return
    /// <see cref="Task"/> and therefore can accept a <see cref="CancellationToken"/> parameter as well. Dependency
    /// injection is also supported - any other parameters will be resolved from the container. Any exceptions thrown

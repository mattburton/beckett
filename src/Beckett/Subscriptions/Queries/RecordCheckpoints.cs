        ...
        const string sql = """
            WITH updated_checkpoints AS (
                ...
                SELECT c.stream_version, c.stream_position, c.subscription_id, c.stream_name, 
                       now(), 
                       CASE WHEN c.stream_version > c.stream_position THEN now() ELSE NULL END
                FROM unnest($1) c
                ON CONFLICT (subscription_id, stream_name) DO UPDATE
                    SET stream_version = excluded.stream_version,
                        updated_at = now(),
                        process_at = CASE WHEN excluded.stream_version > checkpoints.stream_position THEN now() ELSE NULL END
            ...
            inserted_ready AS (
                ...
                WHERE uc.status = 'active' AND uc.process_at IS NOT NULL AND uc.stream_version > uc.stream_position

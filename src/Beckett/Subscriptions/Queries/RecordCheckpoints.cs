    ...
    public async Task<int> Execute(NpgsqlCommand command, CancellationToken cancellationToken)
    {
        ...
        const string sql = """
            WITH updated_checkpoints AS (
                INSERT INTO beckett.checkpoints (stream_version, stream_position, subscription_id, stream_name, updated_at)
                SELECT c.stream_version, c.stream_position, c.subscription_id, c.stream_name, now()
                FROM unnest($1) c

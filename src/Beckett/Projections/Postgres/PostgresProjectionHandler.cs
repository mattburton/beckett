...
public static class PostgresProjectionHandler<TProjection, TState, TKey> where TProjection : IPostgresProjection<TState, TKey>
    where TState : class, IApply, new()
{
    ...
    )
    {
        ...
        var databaseBatch = new NpgsqlBatch();
        ...
        foreach (var keyBatch in  messagesToApply.GroupBy(x => configuration.GetKey(x)))
        {
            ...
            if (lastConfiguration.Action == ProjectionAction.Delete)
            {
                ...
                databaseBatch.BatchCommands.Add(projection.Delete(key));
    ...
    private static NpgsqlBatchCommand? HandleDelete(
        TProjection projection,
        TKey key,
